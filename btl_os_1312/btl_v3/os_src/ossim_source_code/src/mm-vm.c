...
int __free(struct pcb_t *caller, int rgid)
{
  ...
  struct vm_rg_struct *unused_rg = init_vm_rg(rgnode.rg_start, rgnode.rg_end, rgnode.vmaid);
  struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, rgnode.vmaid);

  enlist_vm_rg_node(&cur_vma->vm_freerg_list, unused_rg);
...
int pgalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  int __alloc_val = __alloc(proc, 0, reg_index, size, &addr);
...
int pgmalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  printf("__malloc\n");
  pthread_mutex_lock(&lock);
  int __malloc_val = __alloc(proc, 1, reg_index, size, &addr);
  pthread_mutex_unlock(&lock);
...
{
  pthread_mutex_lock(&lock);
  int __free_val = __free(proc, reg_index);
  pthread_mutex_unlock(&lock);
  return __free_val;
}
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    // BITS 0->4 swap type : 0
    // BITS 5->25 offset: swpfpn
...
int __read(struct pcb_t *caller, int rgid, int offset, BYTE *data)
{
  ...
  if (((vmaid == 0) && (currg->rg_start + offset >= currg->rg_end)) || ((vmaid == 1) && (currg->rg_start - offset <= currg->rg_end)) || offset < 0)
    return -1; // check offset trong mem
...
{
  pthread_mutex_lock(&lock);
  if (__write(proc, destination, offset, data) == -1)
  {
    pthread_mutex_unlock(&lock);
    return -1;
  }
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz, int *inc_limit_ret)
{
  ...
  *inc_limit_ret = (int)(cur_vma->vm_end - old_end);
