...
int __read(struct pcb_t *caller, int rgid, int offset, BYTE *data)
{
  ...
  if ( ((vmaid == 0) && (currg->rg_start + offset >= currg->rg_end))
  || ((vmaid == 1) && (currg->rg_start - offset <= currg->rg_end)) 
  || offset < 0) return -1; // check offset trong mem
...
int pgwrite(
		...
		uint32_t offset)
{
if(__write(proc, destination, offset, data) == -1) return -1;
#ifdef IODUMP
